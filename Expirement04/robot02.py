#!/usr/bin/python
#This expirement is to make the robot track all four circles

from pid_controller_3.pid import PID, twiddle
from robotlib02 import *
...
class Robot(object):
	...
	def __init__(self, camera):
		self.camera = camera
		self.camera.pwm_tilt = 640
		self.camera.pwm_turn = 450
	...
	def setWheelPWM(self, leftCorrection, rightCorrection):
		...
		if d == None:
			...
			limit = (self.WHEEL_PWM_MAX - 1) * d/maxDist


		leftCorrection = leftCorrection / 10
		if leftCorrection > limit : leftCorrection = limit
		elif leftCorrection < -limit : leftCorrection = -limit
			
		rightCorrection = rightCorrection / 10
		if rightCorrection > limit : rightCorrection = limit
		elif rightCorrection < -limit : rightCorrection = -limit
	...
	def pushForward(self):
		...
		if self.camera.distanceToTarget() > 0 and self.camera.pwm_tilt == 640:
			self.camera.pwm_tilt += 5
			if self.camera.pwm_tilt > 550:
				self.camera.pwm_tilt = 550
			"""
			pwm.setPWM(LEFT_WHEEL, 0, LEFT_ZERO + 5)
			pwm.setPWM(RIGHT_WHEEL, 0, RIGHT_ZERO - 5)

		
		elif self.camera.distanceToTarget() == None and self.camera.set 
...
class Camera(object):
	...
	def setTurn(self):
		...
		if self.pwm_tilt == 550 and self.distanceToTarget() == None:
			self.pwm_turn += 5
			if self.pwm_turn > 680:
				self.pwm_turn = 680
		
		elif self.distanceToTarget() > 0:
			self.pwm_turn -= 5
			if self.pwm_turn < 450:
				self.pwm_turn = 450

		pwm.setPWM(LEFT_RIGHT, 0, self.pwm_turn)
