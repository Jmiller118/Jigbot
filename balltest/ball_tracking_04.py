	...
	if len(cnts) > 0:
		...
		if radius > 10:
			...
			if (dist_to_circle > (y_max / 2 ) ):
				pwm.setPWM(UP_DOWN, 0, even)	
			elif (dist_to_circle > 50  and dist_to_circle <= ( y_max / 2) ):
				pwm.setPWM(UP_DOWN, 0, half)
			elif (dist_to_circle <= 50):
				pwm.setPWM(UP_DOWN, 0, down)
			"""
			if y >= (y_max / 2) + 10:
				current_cam_servo -= 1
				...
				current_cam_servo += 1
				if current_cam_servo > 650:
					current_cam_servo = 650
